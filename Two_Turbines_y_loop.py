...
Y_coord=np.arange(0,1.2,0.1)
YAW= np.arange(0, 35, 1)

Aep_tot=np.zeros((np.size(Y_coord),np.size(YAW)))

Pow_turb=np.zeros((np.size(Y_coord),np.size(YAW),2))
T_n=np.zeros((np.size(Y_coord),np.size(YAW),2))
T_x=np.zeros((np.size(Y_coord),np.size(YAW),2))
T_y=np.zeros((np.size(Y_coord),np.size(YAW),2))
...
for ws in ws_list:
        ...
        for i in Y_coord: #horizontal loop
          cc=cc+1
          counter=-1
        ...
        Y_loop   =[0,min_coord[0].tolist()[0], p25_coord[0].tolist()[0], p50_coord[0].tolist()[0], p75_coord[0].tolist()[0], max_coord[0].tolist()[0]]
        ...
        title='Wind farm efficiency at ws=%0.1f m/s' %ws
        ...
        path_to_folder_contours = 'Simulation results/'+'%0.1f ws/' %ws + 'Y-Loop/'
        ...
        for k in range(0,np.size(Y_coord[Y_loop])): #horizontal loop
            ...
            axs[0].arrow(7,i,1*np.cos(np.deg2rad(yaw[1])),0,color='r', width=0.05,head_length=0.1,length_includes_head=True, zorder=32)
            axs[0].arrow(7,i,0,1*np.sin(np.deg2rad(yaw[1])),color='r', width=0.05,head_length=0.1,length_includes_head=True, zorder=32)
            
            axs[0].grid()
            title = "y*=%.2f and yaw=%.2f °" %(round(i,2),round(j,2))
            ...
            path_to_folder_contours = 'Simulation results/'+'%0.1f ws/' %ws + 'Y-Loop/' + 'Flow maps/'
            # Create the folder if it doesn't exist
            if not os.path.exists(path_to_folder_contours):
                os.makedirs(path_to_folder_contours)
